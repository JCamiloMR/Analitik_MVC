...
import { FileSpreadsheet, Upload, FolderOpen, Eye, RotateCcw, AlertTriangle, CheckCircle2, XCircle } from 'lucide-react';
...
export function DataLoadingPanel() {
    ...
    const loadRecentImports = async () => {
        try {
            ...
            const response = await fetch(
                `${apiBaseUrl}/api/import/history?empresaId=${empresaId}&pagina=1&tamano=5`,
                {
                    method: 'GET',
                    headers: {
    ...
    const handleFileUpload = async (file: File) => {
        ...
        try {
            ...
            const response = await fetch(`${apiBaseUrl}/api/import/excel`, {
                method: 'POST',
                body: formData
                ...
                <div className="p-6 lg:p-8 space-y-8">
                    ...
                    {importResult && (
                        <motion.div
                            ...
                            transition={{ duration: 0.3 }}
                        >
                            <Card className={`border-2 ${importResult.exitoso
                                ...
                                <CardHeader>
                                    <CardTitle className={`flex items-center space-x-2 ${importResult.exitoso ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'
                                        }`}>
                                        ...
                                        {importResult.exitoso && importResult.importId && (
                                            <>
                                                <Button
                                                    variant="outline"
                                                    size="sm"
                                                    onClick={() => window.open(`${apiBaseUrl}/api/import/report/${importResult.importId}`, '_blank')}
                                                        ...
                                                        <td className="py-3 px-4">
                                                            <div className="flex space-x-2">
                                                                <Button
                                                                    ...
                                                                    onClick={() => window.open(`${apiBaseUrl}/api/import/report/${item.id}`, '_blank')}
                                                                >
                                                                    <Eye className="w-4 h-4" />
                                                                    <span>Ver</span>
                                                                </Button>
