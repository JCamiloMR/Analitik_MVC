...
public class ImportController : ControllerBase
{
        ...
        [FromQuery] string? tipoHoja = null)
    {
        ...
        // Normalizar fechas a UTC sin recrear el objeto
        var fechaImportacionUtc = DateTime.SpecifyKind(
            importacion.FechaImportacion,
            DateTimeKind.Utc
        );

        var fechaInicioBase = importacion.FechaInicioEtl.HasValue
            ? DateTime.SpecifyKind(importacion.FechaInicioEtl.Value, DateTimeKind.Utc)
            : fechaImportacionUtc;

        var fechaFinBase = importacion.FechaFinEtl.HasValue
            ? DateTime.SpecifyKind(importacion.FechaFinEtl.Value, DateTimeKind.Utc)
            : DateTime.UtcNow;

        Console.WriteLine("FechaImportacion Kind: " + fechaImportacionUtc.Kind);
        Console.WriteLine("FechaInicioEtl Kind: " + (importacion.FechaInicioEtl?.Kind));
        Console.WriteLine("FechaFinEtl Kind: " + (importacion.FechaFinEtl?.Kind));

        // Calcular ventana de tiempo (±5 segundos)
        var fechaInicio = fechaInicioBase.AddSeconds(-5);
        var fechaFin = fechaFinBase.AddSeconds(5);
        ...
        {
            var idproductos = _dbContext.Productos.Select(p => p.EmpresaId).ToList();
            var queryProductos = _dbContext.Productos
                .Where(p => p.EmpresaId == id_empresa)
            ...
            {
                paginacion.TotalRegistros = totalProductos;
                paginacion.TotalPaginas = (int)Math.Ceiling(totalProductos / (double)tamanoPagina);
            }
        }
        ...
        if (tipoHojaNormalizado == null || tipoHojaNormalizado == "inventario")
        {
            var queryInventario = _dbContext.Inventarios
                .Where(i => i.Producto.EmpresaId == id_empresa)
            ...
            {
                paginacion.TotalRegistros = totalInventario;
                paginacion.TotalPaginas = (int)Math.Ceiling(totalInventario / (double)tamanoPagina);
            }
        }
        ...
        if (tipoHojaNormalizado == null || tipoHojaNormalizado == "ventas")
        {
            var queryVentas = _dbContext.Ventas
                .Where(v => v.EmpresaId == id_empresa)
            ...
            {
                paginacion.TotalRegistros = totalVentas;
                paginacion.TotalPaginas = (int)Math.Ceiling(totalVentas / (double)tamanoPagina);
            }
        }
        ...
        if (tipoHojaNormalizado == null || tipoHojaNormalizado == "financieros")
        {
            var queryFinancieros = _dbContext.DatosFinancieros
                .Where(f => f.EmpresaId == id_empresa)
            ...
            {
                paginacion.TotalRegistros = totalFinancieros;
                paginacion.TotalPaginas = (int)Math.Ceiling(totalFinancieros / (double)tamanoPagina);
            }
        }
